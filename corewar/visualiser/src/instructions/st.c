...
static void write_color(champion_t *champion, long dest, char map[MEM_SIZE])
{
    map[(dest + REG_SIZE - 1) % MEM_SIZE] = champion->color;
    if (REG_SIZE > 1)
        map[(dest + REG_SIZE - 2) % MEM_SIZE] = champion->color;
    if (REG_SIZE > 2) {
        map[(dest + REG_SIZE - 3) % MEM_SIZE] = champion->color;
        map[(dest + REG_SIZE - 4) % MEM_SIZE] = champion->color;
    }
    if (REG_SIZE > 4) {
        map[(dest + REG_SIZE - 5) % MEM_SIZE] = champion->color;
        map[(dest + REG_SIZE - 6) % MEM_SIZE] = champion->color;
        map[(dest + REG_SIZE - 7) % MEM_SIZE] = champion->color;
        map[(dest + REG_SIZE - 8) % MEM_SIZE] = champion->color;
    }
}
...
void i_st(champion_t *champion, corewar_t *corewar)
{
    ...
    if (param[1] == 3) {
        ...
        write_color(champion, champion->PC + swap_16bits(corewar->map,
        (champion->PC + 3)) % IDX_MOD, corewar->color);
    }
