...
static void i_ld_help(char param[4], champion_t *champion, corewar_t *corewar,
    long value)
{
    if (param[0] == 3)
        load_value(&(champion->reg[corewar->map[(champion->PC + 2 + param[0]
        - param[0] / 3 + (param[0] == 2) * 2) % MEM_SIZE] - 1]),
        champion->PC + value % IDX_MOD, corewar->map);
    else
...
void i_ld(champion_t *champion, corewar_t *corewar)
{
    ...
    i_ld_help(param, champion, corewar, value);
